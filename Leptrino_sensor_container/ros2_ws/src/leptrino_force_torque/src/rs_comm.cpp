...
#include <cstring>   // For string functions like memcpy
...
int Comm_Open(const char * dev)
{
  ...
  tcflush(fd, TCIFLUSH); // 确保设备缓冲区清理干净
  delim = 0;
  //デリミタ
  delim = 0;

  //std::cout << "Serial port opened successfully. Starting continuous read..." << std::endl;
  // Continuously read from the serial port and print
  //ContinuousSerialRead();
...
void Comm_Setup(long baud, int parity, int bitlen, int rts, int dtr, char code)
{
  ...
  // 设置读取超时
  tio.c_cc[VMIN] = 0;     // 最少读取字符数
  tio.c_cc[VTIME] = 1;   // 读取超时时间，以 0.1 秒为单位（这里是 1 秒）
...
int Comm_GetRcvData(UCHAR *buff)
{
  ...
  l = strlen((char*)buff);
  printf("Data retrieved from buffer, length: %d\n", l);
  return l;
}
...
int Comm_CheckRcv()
{
  printf("Check receive buffer: p_wr=%d, p_rd=%d\n", p_wr, p_rd);
  return p_wr - p_rd;
}
...
{
    int count = 0;
    char read_buffer[100];  // Buffer to store the data
    while (count < 5)
    {
        int bytes_read = read(fd, read_buffer, sizeof(read_buffer));  // Reading up to 100 bytes
        ...
        if (bytes_read > 0)
        {
            ...
            {
                char buffer[4];
                snprintf(buffer, sizeof(buffer), "%02X ", static_cast<unsigned char>(read_buffer[i]));
                data_str += buffer;
            }
