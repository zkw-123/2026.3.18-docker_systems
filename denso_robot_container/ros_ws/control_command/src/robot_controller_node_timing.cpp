...
#include <cmath>
#include <algorithm>
#include <chrono>   // [TIMING] added
#include <iomanip>  // [TIMING] added
...
private:
  ...
  struct TimingRecord {
    ...
    // command-level
    SteadyClock::time_point t_cmd_received{};
    SteadyClock::time_point t_cmd_parsed{};
    SteadyClock::time_point t_dispatch_start{};
    SteadyClock::time_point t_dispatch_end{};

    // moveit-level (best-effort; some commands may not fill all)
    bool has_plan{false};
    bool has_execute{false};

    SteadyClock::time_point t_plan_start{};
    SteadyClock::time_point t_plan_end{};
    SteadyClock::time_point t_execute_start{};
    SteadyClock::time_point t_execute_end{};
  };

  TimingRecord timing_;  // single-threaded command processing assumed

  static double ms(const SteadyClock::duration &d) {
    return std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(d).count();
  }

  void resetTiming(uint32_t id, const std::string &type) {
    timing_ = TimingRecord{};
    timing_.command_id = id;
    timing_.command_type = type;
  }

  void timingMarkCmdReceived() { timing_.t_cmd_received = SteadyClock::now(); }
  void timingMarkCmdParsed()   { timing_.t_cmd_parsed   = SteadyClock::now(); }
  void timingMarkDispatchStart(){ timing_.t_dispatch_start = SteadyClock::now(); }
  void timingMarkDispatchEnd() { timing_.t_dispatch_end = SteadyClock::now(); }

  void timingMarkPlanStart() {
    if (!timing_.has_plan) {
      timing_.has_plan = true;
      timing_.t_plan_start = SteadyClock::now();
    }
  }
  void timingMarkPlanEnd() {
    if (timing_.has_plan) {
      timing_.t_plan_end = SteadyClock::now();
    }
  }
  void timingMarkExecuteStart() {
    if (!timing_.has_execute) {
      timing_.has_execute = true;
      timing_.t_execute_start = SteadyClock::now();
    }
  }
  void timingMarkExecuteEnd() {
    if (timing_.has_execute) {
      timing_.t_execute_end = SteadyClock::now();
    }
  }

  void printTimingSummary() {
    // Always available:
    const double parse_ms = (timing_.t_cmd_parsed.time_since_epoch().count() > 0 &&
                             timing_.t_cmd_received.time_since_epoch().count() > 0)
                                ? ms(timing_.t_cmd_parsed - timing_.t_cmd_received)
                                : -1.0;

    const double dispatch_ms = (timing_.t_dispatch_end.time_since_epoch().count() > 0 &&
                                timing_.t_dispatch_start.time_since_epoch().count() > 0)
                                   ? ms(timing_.t_dispatch_end - timing_.t_dispatch_start)
                                   : -1.0;

    const double total_ms = (timing_.t_dispatch_end.time_since_epoch().count() > 0 &&
                             timing_.t_cmd_received.time_since_epoch().count() > 0)
                                ? ms(timing_.t_dispatch_end - timing_.t_cmd_received)
                                : -1.0;

    // MoveIt best-effort:
    const double plan_ms = (timing_.has_plan &&
                            timing_.t_plan_end.time_since_epoch().count() > 0 &&
                            timing_.t_plan_start.time_since_epoch().count() > 0)
                               ? ms(timing_.t_plan_end - timing_.t_plan_start)
                               : -1.0;

    const double exec_ms = (timing_.has_execute &&
                            timing_.t_execute_end.time_since_epoch().count() > 0 &&
                            timing_.t_execute_start.time_since_epoch().count() > 0)
                               ? ms(timing_.t_execute_end - timing_.t_execute_start)
                               : -1.0;

    const double cmd_to_exec_start_ms =
        (timing_.has_execute && timing_.t_execute_start.time_since_epoch().count() > 0 &&
         timing_.t_cmd_received.time_since_epoch().count() > 0)
            ? ms(timing_.t_execute_start - timing_.t_cmd_received)
            : -1.0;

    const double plan_to_exec_start_ms =
        (timing_.has_plan && timing_.has_execute &&
         timing_.t_execute_start.time_since_epoch().count() > 0 &&
         timing_.t_plan_end.time_since_epoch().count() > 0)
            ? ms(timing_.t_execute_start - timing_.t_plan_end)
            : -1.0;

    RCLCPP_INFO(this->get_logger(),
                "[Timing #%u | %s] parse=%.3f ms, dispatch=%.3f ms, total=%.3f ms",
                timing_.command_id, timing_.command_type.c_str(),
                parse_ms, dispatch_ms, total_ms);

    if (timing_.has_plan) {
      RCLCPP_INFO(this->get_logger(),
                  "[Timing #%u | %s] plan=%.3f ms",
                  timing_.command_id, timing_.command_type.c_str(), plan_ms);
    }
    if (timing_.has_execute) {
      RCLCPP_INFO(this->get_logger(),
                  "[Timing #%u | %s] cmd->exec_start=%.3f ms, plan_end->exec_start=%.3f ms, exec=%.3f ms",
                  timing_.command_id, timing_.command_type.c_str(),
                  cmd_to_exec_start_ms, plan_to_exec_start_ms, exec_ms);
    }
  }
  // ===================== [TIMING] additions end =====================
                                  ...
                                  double tol_j4_rad)
  {
    ...
    // [TIMING] per-attempt local timing (does not overwrite command-global plan/exec markers unless first time)
    const auto t_attempt_plan_start = SteadyClock::now();
    timingMarkPlanStart();
    bool ok = (move_group_->plan(plan) == moveit::core::MoveItErrorCode::SUCCESS);
    timingMarkPlanEnd();
    const auto t_attempt_plan_end = SteadyClock::now();

    RCLCPP_INFO(this->get_logger(),
                "[Timing #%u | %s] J4 corridor attempt (%.1f deg): plan=%.3f ms",
                timing_.command_id, timing_.command_type.c_str(),
                tol_j4_rad * 180.0 / M_PI,
                ms(t_attempt_plan_end - t_attempt_plan_start));

    if (ok) {
      const auto t_attempt_exec_start = SteadyClock::now();
      timingMarkExecuteStart();
      auto exec = move_group_->execute(plan);
      timingMarkExecuteEnd();
      const auto t_attempt_exec_end = SteadyClock::now();

      RCLCPP_INFO(this->get_logger(),
                  "[Timing #%u | %s] J4 corridor attempt (%.1f deg): exec=%.3f ms",
                  timing_.command_id, timing_.command_type.c_str(),
                  tol_j4_rad * 180.0 / M_PI,
                  ms(t_attempt_exec_end - t_attempt_exec_start));
  ...
  void commandCallback(const std_msgs::msg::String::SharedPtr msg) {
    // [TIMING] command-level entry timing
    command_count_++;
    timingMarkCmdReceived();
    ...
    // [TIMING] bind a new record to this command id/type
    resetTiming(command_count_, type);
    // Note: resetTiming overwrote t_cmd_received; restore it as "received" for this record.
    timing_.t_cmd_received = SteadyClock::now();  // keep same semantic: callback entry
    ...
    timingMarkDispatchStart();

    // Keep existing ROS-time execution time print, but it is not used for profiling deltas.
    ...
    if (type == "pose") {
      ...
      if (iss >> dz) {
        ...
        // [TIMING] InsertStrategy may plan+execute internally; treat as execute block
        const auto t_insert_start = SteadyClock::now();
        timingMarkExecuteStart();
        bool ok = InsertStrategy::execute(*move_group_, this->get_logger(), dz, percent, mode);
        timingMarkExecuteEnd();
        const auto t_insert_end = SteadyClock::now();

        RCLCPP_INFO(this->get_logger(),
                    "[Timing #%u | %s] InsertStrategy::execute block=%.3f ms",
                    timing_.command_id, timing_.command_type.c_str(),
                    ms(t_insert_end - t_insert_start));
      ...
      RCLCPP_INFO(this->get_logger(), "Use one of: pose, move, joints, insert, down, current");
    }
  ...
  void moveToPose(double x, double y, double z) {
    ...
    if (!done) {
      ...
      // [TIMING]
      timingMarkPlanStart();
      const auto t_fallback_plan_start = SteadyClock::now();
      bool plan_success = (move_group_->plan(plan) == moveit::core::MoveItErrorCode::SUCCESS);
      const auto t_fallback_plan_end = SteadyClock::now();
      timingMarkPlanEnd();

      RCLCPP_INFO(this->get_logger(),
                  "[Timing #%u | %s] fallback plan=%.3f ms",
                  timing_.command_id, timing_.command_type.c_str(),
                  ms(t_fallback_plan_end - t_fallback_plan_start));

      if (plan_success) {
        // [TIMING]
        timingMarkExecuteStart();
        const auto t_fallback_exec_start = SteadyClock::now();
        auto exec = move_group_->execute(plan);
        const auto t_fallback_exec_end = SteadyClock::now();
        timingMarkExecuteEnd();

        RCLCPP_INFO(this->get_logger(),
                    "[Timing #%u | %s] fallback exec=%.3f ms",
                    timing_.command_id, timing_.command_type.c_str(),
                    ms(t_fallback_exec_end - t_fallback_exec_start));
  ...
  void moveToPosition(double x, double y, double z) {
    ...
    // [TIMING]
    timingMarkPlanStart();
    const auto t_plan_start = SteadyClock::now();
    bool plan_success = (move_group_->plan(plan) == moveit::core::MoveItErrorCode::SUCCESS);
    const auto t_plan_end = SteadyClock::now();
    timingMarkPlanEnd();

    RCLCPP_INFO(this->get_logger(),
                "[Timing #%u | %s] plan=%.3f ms",
                timing_.command_id, timing_.command_type.c_str(),
                ms(t_plan_end - t_plan_start));

    if (plan_success) {
      // [TIMING]
      timingMarkExecuteStart();
      const auto t_exec_start = SteadyClock::now();
      auto exec_res = move_group_->execute(plan);
      const auto t_exec_end = SteadyClock::now();
      timingMarkExecuteEnd();

      RCLCPP_INFO(this->get_logger(),
                  "[Timing #%u | %s] exec=%.3f ms",
                  timing_.command_id, timing_.command_type.c_str(),
                  ms(t_exec_end - t_exec_start));
  ...
  void moveToDownPose(double x, double y, double z, double yaw_rad = 0.0) {
    ...
    // 关键：垂直向下（示例：roll=pi, pitch=0, yaw=yaw）
    tf2::Quaternion q;
    ...
    RCLCPP_INFO(this->get_logger(),
                "[Timing #%u | %s] down-pose plan=%.3f ms",
                timing_.command_id, timing_.command_type.c_str(),
                ms(t_plan_end - t_plan_start));
    ...
    RCLCPP_INFO(this->get_logger(),
                "[Timing #%u | %s] down-pose exec=%.3f ms",
                timing_.command_id, timing_.command_type.c_str(),
                ms(t_exec_end - t_exec_start));
  ...
  void moveToJoints(const std::vector<double> &joints) {
    ...
    // [TIMING] move() internally does plan+execute; we can only time as a whole block here
    timingMarkExecuteStart();
    const auto t_move_start = SteadyClock::now();
    bool success = static_cast<bool>(move_group_->move());
    const auto t_move_end = SteadyClock::now();
    timingMarkExecuteEnd();

    RCLCPP_INFO(this->get_logger(),
                "[Timing #%u | %s] move_group_->move() block=%.3f ms (plan+exec combined)",
                timing_.command_id, timing_.command_type.c_str(),
                ms(t_move_end - t_move_start));
  ...
  void insertAlongWorldZ(double dz, double percent) {
    ...
    // [TIMING] computeCartesianPath timing (planning-like)
    timingMarkPlanStart();
    const auto t_cart_start = SteadyClock::now();
    double fraction = move_group_->computeCartesianPath(
        waypoints, eef_step, jump_threshold, trajectory);
    const auto t_cart_end = SteadyClock::now();
    timingMarkPlanEnd();

    RCLCPP_INFO(this->get_logger(),
                "[Timing #%u | %s] computeCartesianPath=%.3f ms",
                timing_.command_id, timing_.command_type.c_str(),
                ms(t_cart_end - t_cart_start));
    ...
    if (fraction > 0.9) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "[Timing #%u | %s] cartesian fallback exec=%.3f ms",
                  timing_.command_id, timing_.command_type.c_str(),
                  ms(t_exec_end - t_exec_start));
